...
const GoogleReviewsSection = () => {
  ...
  const reviews = [
    {
      ...
      rating: 5,
      comment: "אני מרגיש צורך מיוחד להביע את תודתי לצוות Elite Design kitchens, בראשותו של רומן. אני מעריך אתכם כאנשי מקצוע מיומנים ביותר. אתם לא רק מוכרים את המוצר שלכם - מטבחים - אתם פותרים את הבעיה של הלקוח שלכם בגישה יצירתית. זה בהחלט היה המקרה איתנו. המטבח שלכם עמד במבחן הזמן, הוכיח את איכותו הגבוהה ונוחותו. וכל העצות שלכם הוכיחו את עצמן כבעלות ערך. ומה שחשוב מאין כמוהו הוא שתשמרו על קשר עם הלקוחות שלכם. אני מנצל הזדמנות זו כדי להמליץ על החברה שלכם לכל מי שבונה מטבח. מוניטין אפשר לצבור, אבל צריך לשמור עליו מדי יום. וזה בדיוק מה שאתם עושים. אני מאחל ל-Elite Design kitchens המשך שגשוג.",
  ...
  return (
    <section id="reviews" className="py-24 bg-[#f8f9fa] relative overflow-hidden" aria-label="Customer reviews">
